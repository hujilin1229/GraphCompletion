...
class Model(object):
    ...
    def lanczos(self, x, L, Fout, K):
        N, M, Fin, B = x.get_shape()
        N, M, Fin, B = int(N), int(M), int(Fin), int(B)
        # Rescale Laplacian and store as a TF sparse tensor. Copy to not modify
        # the shared L.
        x = tf.transpose(x, perm=[1, 2, 3, 0])
        x = tf.reshape(x, (M, Fin * B * N))

        # Lanczos convolution
        result_tensor = x
        last_element = x
        multiplier = L / (L.sum(0) + 1)
        multiplier = scipy.sparse.csr_matrix(multiplier)

        multiplier = multiplier.tocoo()
        indices = np.column_stack((multiplier.row, multiplier.col))
        multiplier = tf.SparseTensor(indices, multiplier.data.astype(np.float32), multiplier.shape)
        multiplier = tf.sparse_reorder(multiplier)

        for i in range(1, K):
            cur_element = tf.sparse_tensor_dense_matmul(multiplier, last_element)
            result_tensor += cur_element
            last_element = cur_element

        result_tensor = tf.reshape(result_tensor, (M, Fin * B, N))
        l_out_tensors = []
        for k_i in range(Fout):
            W = self._weight_variable(
                [M, Fin * B], index=k_i, regularization=True)
            W2 = self._weight_variable(
                [Fin * B, B], index=k_i+Fout, regularization=True)
            l_tensor = []
            for n_i in range(N):
                tensor_i = result_tensor[:, :, n_i]
                tensor_i = W * tensor_i
                tensor_i = tf.matmul(tensor_i, W2)
                tensor_i = tf.expand_dims(tensor_i, -1)
                l_tensor.append(tensor_i)
            Fi_out_tensor = tf.concat(l_tensor, axis=-1)
            Fi_out_tensor = tf.expand_dims(Fi_out_tensor, axis=2)
            l_out_tensors.append(Fi_out_tensor)

        gconvoluted = tf.concat(l_out_tensors, axis=2)
        gconvoluted = tf.transpose(gconvoluted, perm=[3, 0, 2, 1])

        return gconvoluted  # N x M x Fout x B

    def learn_heat(self, x, L, Fout, K):
        N, M, Fin, B = x.get_shape()
        N, M, Fin, B = int(N), int(M), int(Fin), int(B)
        # Rescale Laplacian and store as a TF sparse tensor. Copy to not modify
        # the shared L.
        x = tf.transpose(x, perm=[1, 2, 3, 0])
        x = tf.reshape(x, (M, Fin * B * N))
        # L = L.toarray().astype(np.float32)
        # K = exp(-sigma^2 / (2 * \hat(L)))

        L = scipy.sparse.csr_matrix(L)
        L = graph.rescale_L(L, lmax=2)
        L = L.tocoo()
        div_L = -1 * L.data
        L = scipy.sparse.coo_matrix((div_L, (L.row, L.col)), L.shape)

        # matrix exponential
        multiplier = scipy.sparse.linalg.expm(L)
        t_mult = tf.convert_to_tensor(multiplier.toarray().astype(np.float32))
        # matrix divide exponential

        # Lanczos convolution
        l_result = []
        last_element = x
        l_result.append(tf.expand_dims(last_element, 1))

        for i in range(1, K):
            last_element = tf.matmul(t_mult, last_element)
            l_result.append(tf.expand_dims(last_element, 1))
        result_tensor = tf.concat(l_result, axis=1)

        result_tensor = tf.reshape(result_tensor, (M, K * Fin * B, N))
        l_out_tensors = []
        for k_i in range(Fout):
            W2 = self._weight_variable(
                [K * Fin * B, B], index=k_i, regularization=True)
            l_tensor = []
            for n_i in range(N):
                tensor_i = result_tensor[:, :, n_i]
                tensor_i = tf.matmul(tensor_i, W2)
