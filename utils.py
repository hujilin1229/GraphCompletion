...
class LSM_Loader(object):
    def __init__(self, sname, mode, target, sample_rate, win_size,
        ...
        val_data_dict_file = os.path.join(data_dir, 'LSM_val_data_dict.pickle')
        test_data_dict_file = os.path.join(
            data_dir, 'LSM_test_data_dict.pickle')
        ...
        self.row_ind, self.col_ind = \
            self.data_generator(base_dir, data_dir, sname, mode, target,
                                ...
                                s_date, e_month, e_date, data_rm)
        print("Loading data...")
        self.val_data_dict = pklLoad(val_data_dict_file)
        self.test_data_dict = pklLoad(test_data_dict_file)
    ...
    def data_generator(self, base_dir, data_dir, sname, mode, target,
        ...
        dict_edge_node, link_ids, self.W, self.Lap, self.D = \
            dataset.construct_node_graph()

        val_data_dict_file = os.path.join(data_dir, 'LSM_val_data_dict.pickle')
        test_data_dict_file = os.path.join(
            data_dir, 'LSM_test_data_dict.pickle')

        if not os.path.exists(val_data_dict_file) or \
                not os.path.exists(test_data_dict_file):
            ...
            if target == 'avg':
                self.y_scaler = dict_normal['velocity']
                mean_y = np.mean(train_labels, axis=0)
            else:
                self.y_scaler = None
                mean_y = mean_gt(train_labels)
            nan_pos = np.isnan(mean_y)
            mean_y[nan_pos] = 0.
            print("[T]: Has Nan", np.sum(np.isnan(mean_y)))
            ...
            val_vel_list, test_vel_list = \
                train_test_split(test_data, test_labels, test_labels_weight,
                                 test_counts, test_vel_lists, test_size=0.8)

            val_data_dict = dataset.prepare_lsm_learning_data(
                val_x, val_y, val_y_weight, val_count, mean_y, val_vel_list)
            test_data_dict = dataset.prepare_lsm_learning_data(
                test_x, test_y, test_y_weight, test_count, mean_y, test_vel_list)

            pklSave(val_data_dict_file, val_data_dict)
            pklSave(test_data_dict_file, test_data_dict)

        return  dataset.row_ind, dataset.col_ind